...
import {UsersQueryRep} from "../../QueryRep/usersQueryRep";
import {TokenBlackListRepository} from "../../Repository/tokenBlackListRepository";
import {SessionsQueryRep} from "../../QueryRep/sessionsQueryRep";
import {inject} from "inversify";
import {AccessTokenPayloadType, UserViewType} from "../../../Types/Types";
...
export class AuthorizationCheck {
    ...
    accessTokenCheck = async (req: Request, res: Response, next: NextFunction) => {
        ...
        const isTokenValid: AccessTokenPayloadType | null = req.headers.authorization ? await jwtService.verifyAccessToken(req.headers.authorization.split(" ")[1]) : null
        ...
        const neededUser: UserViewType | null = await this.usersQueryRep.findUserById(isTokenValid.id)
